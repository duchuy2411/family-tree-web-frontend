...
import {selectFetchingCurrent, getDetailPerson, selectPerson , REFRESH_CURRENT_PERSON } from '../../Home/homeSlice';
...
export default function ListMember(props) {
  ...
  const formatSpouse = (value) => {
    ...
    const rs =  _.join(_.transform(value, (result, ele) => result.push(ele.name), []), ' ');
    return rs;
  }
          ...
          <TableBody>
            {list.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
              return (
                ...
                >
                  {columns.map((column) => {
                    const value = column.id === 'spouses'
                      ? formatSpouse(row[column.id])
                      : row[column.id];
                    return (
