...
import { updateTree, selectTree, selectTrees, fetchTreesAndSetCurrent, deleteTree } from '../Home/homeSlice';
...
const TreeManagement = () => {
  ...
  useEffect(async () => {
    ...
    if (!currentTree || Object.keys(currentTree).length === 0) dispatch(fetchTreesAndSetCurrent(id));
  }, [])
  ...
  const handleChange = (e, label) => {
    switch (label) {
      case 'name': { setFormTree({ ...formTree, name: e.target.value}); break; };
      case 'description': { setFormTree({ ...formTree, description: e.target.value }); break; };
      case 'publicMode': { setPublicMode({ ...formTree, publicMode: e.target.value }); break; };
    }
  }
  ...
  function createData(obj, classes = null) {
    return {
      ...
      gender: _.get(obj, 'gender', '') === 'Male' ? (<span className={classes.blockMale}></span>) : (<span className={classes.blockFemale}></span>) ,
  ...
  const handleChangeFormSearch = (e, label) => {
    switch (label) {
      ...
      case 'lastName': { setValSearch({ ...valSearch, lastName: e.target.value }); break;}
      case 'age': {
  ...
  const handleSubmitSearch = (age = null) => {
    ...
    const newList = _.filter(originList, ele =>
      ...
      ((valAge === 80 || `${valAge}` === "0") ? (ele.age >= valAge - 9 || ele.age === '') : ((ele.age <= parseInt(valAge)) && (ele.age > parseInt(valAge) - 10))
    ));
    setList([...newList]);
  }
  ...
  return (
    <div className={classes.rootAccording}>
      ...
      <Accordion className={classes.section}>
        ...
        <AccordionDetails className={classes.details}>
          <Contribute
            owner={currentTree.owner}
            editors={currentTree.editors}
          />
        </AccordionDetails>
      </Accordion>
